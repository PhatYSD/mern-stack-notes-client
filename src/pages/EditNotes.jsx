...
const EditNotes = () => {
    ...
    const handerSave = () => {
        setLoading(true);
        fetch(`https://energetic-erin-salmon.cyclic.app/api/notes/${id}`, {
            method: "PUT",
            headers: {
    ...
    useEffect(() => {
        ...
        fetch(`https://energetic-erin-salmon.cyclic.app/api/notes/${id}`)
            .then(res => res.json())
            .then(res => {
                const { title, description, author } = res.data

                setDatas({ title, description, author });
            })
            .catch(error => {
                console.log(error);
                alert(`Error please chack console.`);
    ...
    return (
        <div className="w-full my-4 flex flex-col items-center">
            <h1 className="mb-4 text-4xl font-extralight">Edit Note</h1>
